...
function getConnectorConfig(config, version, description, logo) {
  return {
    ...
    reference: `plugins%2Fconnectors%2F${
      config.usage === "node" ? "node" : "data"
    }%2F${config.id}`,
...
async function loadConnector(
  ...
  try {
    // Sign in to firebase.
    const firebaseSignInResponse = await fetchModule.default(
    ...
    if (!firebaseSignInResponse.ok) {
      console.log(
        firebaseSignInResponse.status,
        firebaseSignInResponse.statusText,
        await firebaseSignInResponse.text()
      );
      return false;
    }
    ...
    const firebaseUpsertResponse = await fetchModule.default(
      `https://europe-west1-${firebaseProjectId}.cloudfunctions.net/api/plugins`,
      {
        body: JSON.stringify(
          getConnectorConfig(
            config,
    ...
    if (!firebaseUpsertResponse.ok) {
      console.log(
        firebaseUpsertResponse.status,
        firebaseUpsertResponse.statusText,
        await firebaseUpsertResponse.text()
      );
      return false;
    }
    ...
    const sanityUpsertResponse = await fetchModule.default(
      "https://yxr5xjfo.api.sanity.io/v2021-06-07/data/mutate/library-production",
      {
    ...
    if (!sanityUpsertResponse.ok) {
      console.log(
        sanityUpsertResponse.status,
        sanityUpsertResponse.statusText,
        await sanityUpsertResponse.text()
      );
      return false;
    }
