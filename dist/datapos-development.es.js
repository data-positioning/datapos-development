...
async function S() {
  try {
    ...
    o.name && (e.id = o.name.replace("@datapos/", "")), o.version && (e.version = o.version), await n.writeFile("config.json", JSON.stringify(e, void 0, 4), "utf8"), console.info("✅ Configuration built.");
  } catch (o) {
    ...
    async function i(r, t) {
      ...
      for (const c of t) {
        ...
        try {
          ...
          if (d.isDirectory()) {
            const p = await n.readdir(l), u = { childCount: p.length, name: `${c}`, typeId: "folder" };
...
async function j() {
  try {
    ...
    a ? console.info(`ℹ️  Supports ${a} usage.`) : console.warn("⚠️  No usage identified."), o.name && (e.id = o.name), e.operations = f, e.usageId = a, o.version && (e.version = o.version), await n.writeFile("config.json", JSON.stringify(e, void 0, 4), "utf8"), console.info("✅ Connector configuration built.");
  } catch (o) {
...
async function x() {
  try {
    ...
    o.name && (e.id = o.name), e.operations = r, o.version && (e.version = o.version), await n.writeFile("config.json", JSON.stringify(e, void 0, 4), "utf8"), console.info("✅ Context configuration built.");
  } catch (o) {
...
async function F() {
  try {
    ...
    const o = JSON.parse(await n.readFile("package.json", "utf8")), e = JSON.parse(await n.readFile("config.json", "utf8")), i = await n.readFile("src/index.ts", "utf8"), s = /^\s{4}(?:async\s+)?(private\s+)?(?:public\s+|protected\s+)?([A-Za-z_]\w*)\s*\(/gm, r = [...i.matchAll(s)].filter((t) => !t[1] && t[2] !== "constructor").map((t) => t[2]);
    o.name && (e.id = o.name), e.operations = r, o.version && (e.version = o.version), await n.writeFile("config.json", JSON.stringify(e, void 0, 4), "utf8"), console.info("✅ Informer configuration built.");
  } catch (o) {
...
async function C() {
  try {
    ...
    const o = JSON.parse(await n.readFile("package.json", "utf8")), e = JSON.parse(await n.readFile("config.json", "utf8")), i = await n.readFile("src/index.ts", "utf8"), s = /^\s{4}(?:async\s+)?(private\s+)?(?:public\s+|protected\s+)?([A-Za-z_]\w*)\s*\(/gm, r = [...i.matchAll(s)].filter((t) => !t[1] && t[2] !== "constructor").map((t) => t[2]);
    o.name && (e.id = o.name), e.operations = r, o.version && (e.version = o.version), await n.writeFile("config.json", JSON.stringify(e, void 0, 4), "utf8"), console.info("✅ Presenter configuration built.");
  } catch (o) {
    console.error("❌ Error building context configuration.", o);
  }
}
...
async function T(o) {
  try {
    ...
    async function s(r, t = "") {
      const f = await n.readdir(r, { withFileTypes: !0 });
      for (const a of f) {
